...
const Auth = {
  ...
  async signInWithApple() {
    ...
    try {
      const result = await window.FirebaseSignIn(window.FirebaseAuth, window.FirebaseAppleProvider);
      Utils.showToast(`Welcome, ${result.user.displayName || 'Guest'}!`, 'success');
    } catch (error) {
      console.error("Auth error:", error);
      Utils.showToast('Authentication failed', 'error');
    ...
    if (!user) {
      section.innerHTML = `
        <div class="auth-container" style="text-align:center; padding: 40px 20px;">
          ...
          <p style="color:var(--text-muted);margin-bottom:28px;">Sign in to manage your orders</p>
          <div class="auth-buttons-group">
            ...
            <button class="btn btn-apple" onclick="Auth.signInWithApple()">
              <svg style="width:20px;height:20px;fill:currentColor;" viewBox="0 0 24 24">
                <path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.81-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.82M15.97 4.17c.66-.81 1.11-1.93.99-3.06-1 .04-2.22.67-2.94 1.51-.63.73-1.18 1.87-1.03 2.98.87-.07 2.32-.62 2.98-1.43z"/>
              </svg>
              Sign in with Apple
            </button>
          </div>
